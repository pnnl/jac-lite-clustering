...
/**
* Interface to define objects that map long keys to long values
*
...
public interface LongLongMap {
    ...
    * -1L if the key is not contained.  Since -1L is a perfectly valid value
    ...
    *           -1L if the map does not contain the key.
    *
    ...
    * @return the previous value paired to the given key, or -1L if no
    ...
    * @param key the key.
    * @return the previous value paired to the given key, or -1L if no
    *   previous mapping was present.  (A -1L return does not
    *   <i>necessarily</i> prove that no mapping was present: the key may have
    *   been explicitly mapped to -1L.)
