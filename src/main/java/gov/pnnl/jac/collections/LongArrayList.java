...
/**
 * <p><tt>LongArrayList</tt> is an implementation of <tt>LongList</tt> that
 ...
 * @version 1.0
 */
public class LongArrayList implements LongList, Cloneable {
  ...
  /**
   * Performs a binary search of the data for a given key.
   * In order for this to work correctly, the data must be
   * sorted.
   * 
   * @param key
   * @return
  ...
  public int binarySearch(long key) {
	  final long[] a = this.mData;
	  int low = 0;
	  int high = this.mSize - 1;
  ...
  public boolean removeAll(long[] values) {
    if (!isEmpty()) {
      int n = values.length;
      if (n > 0) {
        long[] sortedValues = new long[n];
        System.arraycopy(values, 0, sortedValues, 0, n);
        Arrays.sort(sortedValues);
        int keep = 0;
        long[] newData = new long[mData.length];
        for (int i=0; i<mSize; i++) {
          long v = mData[i];
   ...
   */
  public int indexOf(int startIndex, long value) {
   ...
   * the specified object is an <tt>LongList</tt> of the same size and with the
   * same elements in the same order as this list.  The other list may be
   * a different implementation class of <tt>LongList</tt> and still be
   * considered equal.
   * @param o Object
   * @return boolean
   */
  public boolean equals(Object o) {
    ...
    return ListUtils.checkEqual(this, (LongList) o);
  }
   ...
   */
  public Object clone() {
    LongArrayList clone = null;
    try {
      clone = (LongArrayList) super.clone();
      clone.mData = new long[this.mData.length];
  ...
  private class LItr implements LongListIterator {
    ...
    public void remove() {
        ...
        checkForComodification();
        try {
            LongArrayList.this.removeAt(mLastRetNdx);
    ...
    public void set(long value) {
        ...
        checkForComodification();
        try {
            LongArrayList.this.set(mLastRetNdx, value);
            mExpectedModCount = mModCount;
        } catch(IndexOutOfBoundsException e) {
            throw new ConcurrentModificationException();
        }
    ...
    public void add(long value) {
        checkForComodification();
        try {
            LongArrayList.this.add(mCursor++, value);
            mLastRetNdx = -1;
            mExpectedModCount = mModCount;
        } catch(IndexOutOfBoundsException e) {
            throw new ConcurrentModificationException();
