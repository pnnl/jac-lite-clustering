...
/**
 * <p><tt>FloatArrayList</tt> is an implementation of <tt>FloatList</tt> that
...
public class FloatArrayList extends AbstractFloatList {
  ...
  public boolean addAll(int index, float[] values) {
	  return addAll(index, new FloatArrayList(values));
  }
  ...
  public boolean retainAll(FloatCollection c) {
    ...
    if (!isEmpty()) {
        ...
        if (sz == 0) {
          ...
          FloatSet fset = c instanceof FloatSet ? ((FloatSet) c) : new FloatHashSet(c);
          
          int keep = 0;
          float[] newData = new float[mData.length];
  ...
  public FloatArrayList clone() {
    FloatArrayList clone = null;
    try {
      // The AbstractFloatList method _clone() just calls Object.clone().
      clone = (FloatArrayList) super._clone();
      clone.mData = new float[this.mData.length];
