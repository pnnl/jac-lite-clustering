...
/**
 * Interface to define objects that map object keys to integer values.  This
 ...
 * simpler to code and less <i>troublematic</i> as a former colleague used to say.
 *
 ...
 * @version 1.0
 */
public interface ObjectIntMap<E> {
  ...
  /**
   * Returns <tt>true</tt> if the receiver maps the specifed Object key
   * to an integer value.
   *
   ...
   * <tt>-1</tt> if the key is not contained.  Since a key may be
   * mapped to <tt>-1</tt>, such a return value does not
   ...
   *           <tt>-1</tt> if the map does not contain the key.
   *
   ...
   *   been explicitly mapped to <tt>-1</tt>.)
  ...
  // Bulk Operations
  /**
   * Adds all key-value mapping from the specified <tt>ObjectIntMap</tt> to the
   ...
   *            <tt>null</tt>.
   */
  void putAll(ObjectIntMap<? extends E> map);
  ...
  /**
   * A map entry (key-value pair).  The <tt>IntObjectMap.entrySet</tt> method returns
