...
/**
 * <p>Implementation of <tt>LongSet</tt> which uses hashing to add and
 * remove values.</p>
 *
...
public class LongHashSet implements LongSet {
  ...
  public LongHashSet (int initialCapacity, float loadFactor) {
    mMap = new LongLongHashMap(initialCapacity, loadFactor);
  }
  ...
  public boolean retainAll(long[] values) {
    ...
    LongHashSet tmpSet = new LongHashSet(2*n);
    for (int i=0; i<n; i++) {
   ...
   * @return int - the number of unique long values contained in the set.
   */
  public int size() {
    return mMap.size();
  }
   ...
   * @return long[] - an array containing the values contained in the set.
   */
  public long[] toArray() {
    return mMap.keys();
  }
  ...
  public LongSet unionWith(LongSet other) {
    ...
    LongSet newSet = new LongHashSet(initialCap);
   ...
   *   receiver.
   */
  public LongSet intersectionWith(LongSet other) {
    LongSet newSet = new LongHashSet();
   ...
   *   receiver.
   */
  public LongSet xorWith(LongSet other) {
    LongSet newSet = new LongHashSet();
    long[] values = this.toArray();
    int n = values.length;
    for (int i=0; i<n; i++) {
   ...
   * <tt>MapUtils.computeHash(LongSet)</tt>.
   * @return int
   */
  public int hashCode() {
  ...
  /**
   * Check equality with another <tt>LongSet</tt>.
   * @param o Object
  ...
  public boolean equals(Object o) {
    if (o instanceof LongSet) {
      return MapUtils.checkEqual(this, (LongSet) o);
    }
    return false;
  }
